...
pipeline {
    ...
    stages {
        ...
        stage('Linting'){
            steps{
                echo 'Linting..'
                sh 'cppcheck -v --enable=all --style=kr --indent=spaces=4 --indent-namespaces --lineend=linux --min-conditional-indent=0 --std=c++11 --xml --xml-version=2 -I include/  --enable=style  src/ include/ test/ 2> result_cppcheck.xml'
